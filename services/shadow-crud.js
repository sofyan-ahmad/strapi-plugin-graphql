...
const buildAssocResolvers = model => {
  ...
  const redisClient = redis(strapi);
      ...
      switch (nature) {
        ...
        case 'manyToManyMorph': {
          resolver[alias] = async obj => {
            ...
            if (fromCache) {
              entry = JSON.parse(fromCache);
            } else {
              entry = await strapi.query(model.uid).findOne(params, [alias]);
        ...
        default: {
          resolver[alias] = async (obj, options) => {
            ...
            if (['oneToOne', 'oneWay', 'manyToOne'].includes(nature)) {
              ...
              const cacheKey = `${model.info.name.toLowerCase()}:${foreignId}:${JSON.stringify(params).replace(/"/g, "'")}`;

              const fromCache = await redisClient.get(cacheKey);

              let result;
              if (fromCache) {
                result = JSON.parse(fromCache);
              } else {
                const query = {
                  single: true,
                  filters: {
                ...
                result = await loader.load(query).then((r) => assignOptions(r, obj));
            ...
            if (
              ...
              if (fromCache) {
                ...
                result = await loader.load({ filters }).then((r) => assignOptions(r, obj));
            ...
            if (
              ...
              if (Array.isArray(obj[alias])) {
                ...
                const cacheKey = `${model.info.name.toLowerCase()}:${JSON.stringify(params).replace(/"/g, "'")}`;
